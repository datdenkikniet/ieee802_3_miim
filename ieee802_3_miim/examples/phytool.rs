...
use ieee802_3_miim::{
    mdio::PhyAddress,
    registers::{
        auto_negotiation::{AutonegotiationAdvertisement, AutonegotiationLinkPartnerAbility},
        leader_follower::{LeaderFollowerControl, LeaderFollowerStatus},
        BasicControl, ExtendedStatus,
    },
...
impl std::fmt::Display for FullAdvertisedCaps {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let pm = |supported, name: &str| {
            if supported {
                format!("{name} ")
            } else {
                String::new()
        ...
        f.write_str(&pm(self._10base_t_hd, "10BASE-T-HD"))?;
        f.write_str(&pm(self._10base_t_fd, "10BASE-T-FD"))?;
        f.write_str(&pm(self._100base_tx_fd, "100BASE-TX-HD"))?;
        f.write_str(&pm(self._100base_tx_fd, "100BASE-TX-FD"))?;
        f.write_str(&pm(self._1000base_t_hd, "1000BASE-T-HD"))?;
        f.write_str(&pm(self._1000base_t_fd, "1000BASE-T-FD"))?;
        f.write_str(&pm(self._100base_t4, "10BASE-T4"))?;
        Ok(())
    }
}
...
fn main() {
    ...
    if command == "link_status" {
        ...
        let fmt_print = if let Some(reg) = reg {
            ...
            let lf_ad: LeaderFollowerControl = mdio.read();

            let ad = FullAdvertisedCaps {
                _10base_t_hd: ad._10base_t_hd(),
                _10base_t_fd: ad._10base_t_fd(),
                _100base_tx_hd: ad._100base_tx_hd(),
                _100base_tx_fd: ad._100base_tx_fd(),
                _1000base_t_hd: lf_ad._1000base_t_hd(),
                _1000base_t_fd: lf_ad._1000base_t_fd(),
                _100base_t4: ad._100base_t4(),
            };

            let lp_abilities: AutonegotiationLinkPartnerAbility = mdio.read();
            let lp_caps = lp_abilities.technology_ability();
            let lf_stat: LeaderFollowerStatus = mdio.read();

            let lp_ad = FullAdvertisedCaps {
                _10base_t_hd: lp_caps._10base_t_hd(),
                _10base_t_fd: lp_caps._10base_t_fd(),
                _100base_tx_hd: lp_caps._100base_tx_hd(),
                _100base_tx_fd: lp_caps._100base_tx_fd(),
                _1000base_t_hd: lf_stat._1000base_t_hd(),
                _1000base_t_fd: lf_stat._1000base_t_fd(),
                _100base_t4: lp_caps._100base_t4(),
            };

            [
                ("Link state", link_state),
                ("Advertised capabilities", format!("{ad}")),
                ("Link partner capabilities", format!("{lp_ad}")),
            ]
            .to_vec()
        };
    ...
    match register.get() {
        ...
        1 => {
            ...
            [
                ...
                ("100BASE-X FD capable", status._100base_x_fd().to_string()),
            ]
            .to_vec()
        }
        15 => {
